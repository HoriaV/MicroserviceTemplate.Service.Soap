...
namespace MicroserviceTemplate.Service.SoapAdapter
{
    ...
    /// </summary>
    public class SoapRouteResolver : IRouteResolver
    {
        ...
        public ResolveResult Resolve(NancyContext context)
        {
            ...
            if (context.Items.ContainsKey(Utils.SoapActionKey))
            {
                pathDecoded = HttpUtility.UrlDecode(HttpUtility.UrlEncode(pathDecoded + $"/{context.Items[Utils.SoapActionKey]}"));
            }
            return Resolve(context, pathDecoded);
        }
        ...
        public ResolveResult Resolve(NancyContext context, string pathDecoded)
        {
